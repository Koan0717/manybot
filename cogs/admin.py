...
async def build_db_diagnosis_embed(guild_id: int, refresh: bool = False) -> discord.Embed:
    """データベース診断の結果を Embed にまとめる。"""
    ...
    if info["url_is_blank"]:
        verdict = (
            ...
            "正しい接続URLを再設定してから、`refresh: True` を付けて再実行してください。"
        )
...
class AdminGroup(app_commands.Group):
    ...
    async def db_diagnose(self, interaction: discord.Interaction, refresh: bool = False):
        await interaction.response.defer(ephemeral=True)
        try:
            embed = await build_db_diagnosis_embed(interaction.guild.id, refresh=refresh)
        except Exception as e:
    ...
    @is_admin_or_banker()
    @app_commands.describe(amount="金額", users="付与するユーザー達（メンションまたはIDを複数指定）", role="付与するロール")
    async def manual_issue(self, interaction: discord.Interaction, amount: int, users: str = None, role: discord.Role = None):
        await interaction.response.defer()
        if amount <= 0:
            return await interaction.followup.send("❌ 1以上の金額を指定してください。", ephemeral=True)
            
        if not users and not role:
            return await interaction.followup.send("❌ `users` または `role` のいずれかを指定してください。", ephemeral=True)
        ...
        if users:
            ...
            if not user_ids:
                await interaction.followup.send("❌ ユーザーを正しく指定してください（メンションまたはID）。", ephemeral=True)
            else:
                success_users = []
                ...
                if success_users:
                    ...
                    await interaction.followup.send("❌ 指定されたユーザーがサーバー内に見つかりませんでした。", ephemeral=True)

        # ロール指定がある場合
        if role:
            members = role.members
            if not members:
                await interaction.followup.send(f"❌ {role.name} ロールを持つメンバーが見つかりませんでした。", ephemeral=True)
            else:
                success_count = 0
...
class Admin(commands.Cog):
    ...
    @commands.command(name="db診断", aliases=["dbcheck", "db_check"])
    @commands.has_permissions(administrator=True)
    async def db_diagnose_prefix(self, ctx, refresh: str = None):
        """スラッシュコマンドの同期を待たずに使えるデータベース診断。

        使い方: !db診断  /  接続先を読み直す場合: !db診断 refresh
        """
        if ctx.guild is None:
            return await ctx.send("サーバー内で実行してください。")
        do_refresh = str(refresh or "").lower() in ("refresh", "true", "1", "再読込")
        try:
