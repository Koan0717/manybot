...
def has_event_manager_role(bot, user: discord.Member):
    if isinstance(user, discord.User) or not hasattr(user, 'roles'):
        return False
    event_manager_role_ids = get_setting(bot, "EVENT_MANAGER_ROLE_IDS") or []
...
class Points(commands.Cog):
    ...
    async def cog_load(self):
        self.bot.tree.add_command(self.PointGroup)

    async def cog_unload(self):
        self.bot.tree.remove_command("ポイント")
    ...
    async def balance(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        if target_user != interaction.user and not (has_admin_role(self.bot, interaction.user) or has_event_manager_role(self.bot, interaction.user)):
    ...
    async def add(self, interaction: discord.Interaction, users: str, amount: int):
        if not (has_admin_role(self.bot, interaction.user) or has_event_manager_role(self.bot, interaction.user)):
        ...
        if hasattr(config, 'send_economy_log'):
