...
export default function ChannelSelect({
  ...
  // 削除されたチャンネルなど、一覧に無いIDが設定に残っているとき。表示されず解除できなくなるので別に出す
  const unknownIds = channels.length ? valArray.filter((id) => !channels.some((x) => String(x.id) === id)) : [];
        ...
        <div className="overflow-y-auto p-3 flex-1 space-y-1 custom-scrollbar min-h-[220px]">
          ...
          {unknownIds.map((id) => (
            ...
            >
              ...
              <span className="font-tech text-amber-300 truncate">⚠ 見つからないチャンネル（削除済みの可能性） ID: {id}</span>
    ...
    <div className={className}>
      ...
      >
        <div className="flex flex-wrap gap-1.5 items-center flex-1 min-w-0">
          {selectedChannels.length === 0 && unknownIds.length === 0 ? (
