...
export default function RoleSelect({
  ...
  if (loading) {
    return (
      <div className="bg-zinc-900 border border-zinc-700 rounded-lg p-2 text-zinc-500 text-sm h-11 flex items-center">
        ロール読み込み中...
      </div>
    );
  }
      ...
      <div className="relative z-10 w-full max-w-lg bg-zinc-900 border border-zinc-700 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="overflow-y-auto p-3 flex-1 space-y-1 custom-scrollbar min-h-[220px]">
          ...
          {filteredRoles.length === 0 ? (
            ...
            filteredRoles.map((r) => {
              const isSelected = valArray.includes(String(r.id));
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <Shield className="w-3.5 h-3.5 flex-shrink-0" style={{ color: roleColor }} />
                    <span className="font-tech truncate font-bold" style={{ color: roleColor }}>
                      @{r.name}
                    </span>
                  </div>
        ...
        <div className="flex flex-wrap gap-1.5 items-center flex-1 min-w-0">
          {selectedRoles.length === 0 ? (
            ...
            selectedRoles.map((r) => (
              <span
                key={r.id}
                className="bg-zinc-800 text-xs px-2.5 py-1 rounded border border-zinc-700 font-bold flex items-center gap-1 font-tech"
