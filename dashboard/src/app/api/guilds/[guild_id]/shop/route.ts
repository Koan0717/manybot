...
export async function POST(
  ...
  try {
    ...
    if (action === 'add') {
      ...
      const { name, usage, price, target_role_ids, reward_role_ids, duration_days, is_eval_extend, extend_days } = item;
      const result = await pool.query(
      ...
      const item = body.item || body;
      const { item_id, name, usage, price, target_role_ids, reward_role_ids, duration_days, is_eval_extend, extend_days } = item;
      await pool.query(
