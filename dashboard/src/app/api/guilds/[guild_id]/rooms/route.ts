...
export async function GET(
  ...
  try {
    const result = await pool.query(
      "SELECT setting_key, setting_value FROM bot_settings WHERE guild_id = $1 AND setting_key IN ('ROOM_PRICES', 'ROOM_PANEL_CONFIGS', 'ENABLE_PRICE_MAIN_SUB', 'ENABLE_PRICE_NEW_MEMBER', 'ENABLE_PRICE_DOWNGRADE', 'ENABLE_PRICE_VIOLATOR', 'ENABLE_FREE_INN_MAIN_SUB', 'DISABLE_12H_ROOMS', 'DISABLE_24H_ROOMS', 'DISABLE_12H_INN', 'DISABLE_24H_INN', 'DISABLE_12H_LUXURY', 'DISABLE_24H_LUXURY', 'DISABLE_6H_GAMEVC', 'DISABLE_12H_GAMEVC', 'DISABLE_6H_GAMBLEVC', 'DISABLE_12H_GAMBLEVC', 'ROOM_ACCESS_LOW_EVAL_INN_TEXT', 'ENABLE_GAME_VC_HOURLY', 'ROOM_STREAM_INN', 'ROOM_STREAM_LUXURY_INN', 'ROOM_STREAM_GAME_VC', 'ROOM_STREAM_GAMBLE_VC', 'ROOM_STREAM_CUSTOM_VC')",
      [guildId]
    );
    ...
    let toggles = {
      ...
      // 部屋ごとの画面共有（未設定は許可）。Bot の cogs/rooms.py が作成時に反映する
