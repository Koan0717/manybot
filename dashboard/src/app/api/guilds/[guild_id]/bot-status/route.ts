  ...
  if (pool) {
    ...
    try {
      ...
      if (!enableRank) {
        modules['rank'] = {
          configured: false,
          summary: '機能オフ (ランク機能全体が無効化中)',
        };
      } else if (rRes.rows.length > 0 || botSettingsResult.rows.length > 0) {
        ...
        const details = [];
        if (!enableTcRank) details.push('TC集計オフ');
        ...
        modules['rank'] = {
          configured: true,
          summary: `稼働中 (XP集計・ランク機能有効${details.length > 0 ? ` / ${details.join('・')}` : ''})`,
        };
      } else {
        modules['rank'] = {
          configured: true,
