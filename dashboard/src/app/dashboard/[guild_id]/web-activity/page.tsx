...
type RoleAccess = Record<'downgrade' | 'violator', Record<'casino' | 'shop' | 'gacha' | 'games', boolean>>;
  ...
  useEffect(() => {
    ...
    fetch(`/api/guilds/${guildId}/settings`)
      ...
      .then((data: any) => {
        ...
        setRoleAccess({
          downgrade: { casino: ra.downgrade?.casino !== false, shop: ra.downgrade?.shop !== false, gacha: ra.downgrade?.gacha !== false, games: ra.downgrade?.games !== false },
          violator: { casino: ra.violator?.casino !== false, shop: ra.violator?.shop !== false, gacha: ra.violator?.gacha !== false, games: ra.violator?.games !== false },
        });
      })
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch(`/api/guilds/${guildId}/settings`, {
        ...
        body: JSON.stringify({ WEB_GAMES_ENABLED: enabled, WEB_SHOP_ENABLED: shopEnabled, WEB_GACHA_ENABLED: gachaEnabled, WEB_BOARDGAMES_ENABLED: boardGames, WEB_ROLE_ACCESS: roleAccess,
        }),
      });
    ...
    <div className="space-y-6">
      ...
      >
        <div className="border-b border-cyan-500/20 pb-4 mb-2">
          ...
          <p className="text-xs text-gray-500 mt-1">
            ...
            の各ゲームの設定がそのまま使われ、戦績もDiscordのパネルと共通です。
          </p>
        </div>
        {BOARD_GAMES.map((g) => (
          <div key={g.key} className="flex items-center justify-between gap-4 py-3 border-b border-gray-700/60 last:border-0">
            ...
            <label className="relative inline-flex items-center cursor-pointer flex-shrink-0">
              <input
                type="checkbox"
                className="sr-only peer"
                checked={boardGames[g.key]}
                onChange={(e) => setBoardGames((prev) => ({ ...prev, [g.key]: e.target.checked }))}
              />
              <div className="w-14 h-7 bg-gray-700 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-6 after:w-6 after:transition-all peer-checked:bg-cyan-600"></div>
            </label>
          </div>
      ...
      >
        <div className="border-b border-cyan-500/20 pb-4">
          ...
          <p className="text-xs text-gray-500 mt-1">
            OFFにすると、そのメンバーの画面ではそのタブ（カジノ・ショップ・ガチャ・ゲーム）が表示されず、使うこともできません。
