    ...
    <div className="max-w-5xl mx-auto pb-24 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <PageHeader
          icon={Activity}
          title="BOT機能接続状況"
          subtitle="Bot本体・データベース・Discord連携および各設定モジュールの稼働状況を総合診断します"
      ...
      {/* 1. コアサービス接続状況 (画像と同一デザインのメカHUDカード) */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3.5">
          ...
          <div className="mecha-clip-sm bg-black/40 border border-zinc-800 p-4 flex items-center gap-3.5 relative overflow-hidden">
            ...
            <div className="min-w-0 flex-1">
              <div className="font-tech text-xs text-zinc-500 flex items-center gap-2">
                Supabase (Database)
                {data?.db.isDedicated && (
                  <span className="text-[10px] bg-cyan-950/80 text-cyan-300 border border-cyan-800 px-1.5 py-0.2 rounded font-bold">専用DB</span>
                )}
              </div>
              {loading ? (
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-mecha text-sm font-bold tracking-wider text-cyan-400 uppercase flex items-center gap-2">
            ...
            各機能モジュールの設定・連携状況 (Feature Modules)
          </h2>
          <span className="font-tech text-xs text-zinc-500">カードをクリックすると設定画面を開きます</span>
        </div>
