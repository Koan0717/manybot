...
export default function RoomsSettingsPage({ params }: { params: { guild_id: string } }) {
  ...
  // カテゴリごとの「12/24時間ボタンOFF」設定キー一覧
  const durationCategories = [
    { key: 'inn', label: '一般宿', color: 'text-green-400', k12: 'DISABLE_12H_INN', k24: 'DISABLE_24H_INN' },
    { key: 'luxury', label: '高級宿', color: 'text-purple-400', k12: 'DISABLE_12H_LUXURY', k24: 'DISABLE_24H_LUXURY' },
    { key: 'game', label: 'ゲームVC', color: 'text-blue-400', k12: 'DISABLE_12H_GAMEVC', k24: 'DISABLE_24H_GAMEVC' },
    { key: 'gamble', label: '賭博VC', color: 'text-orange-400', k12: 'DISABLE_12H_GAMBLEVC', k24: 'DISABLE_24H_GAMBLEVC' },
  ] as const;
      ...
      <div className="mecha-clip mecha-grid-bg bg-neutral-900/80 border border-zinc-800/80 p-6 shadow-xl mb-8">
        <div className="flex justify-between items-center mb-6 border-b border-zinc-700 pb-2">
          <div>
            ...
            <p className="text-xs text-zinc-400 mt-1">一般宿・高級宿・ゲームVC・賭博VCごとに、購入時・延長時に表示する時間ボタンを個別に制限できます。</p>
          </div>
          <button
          ...
          <div key={cat.key} className={idx < durationCategories.length - 1 ? 'mb-6' : ''}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(['12h', '24h'] as const).map(dur => {
                ...
                const durLabel = dur === '12h' ? '12' : '24';
                const otherDurLabel = dur === '12h' ? '24' : '12';
                return (
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-400 mt-1">
              有効にすると、ゲームVC作成の選択画面に「⏱ 時間指定で通話作成」ボタンが表示され、通話名と利用時間（時間単位）を自由入力してVCを作成できるようになります。料金はゲームVCの12時間料金から1時間あたりの単価を算出して計算されます。OFFにすると、いつも通り12時間/24時間ボタンのみの表示に戻ります。
            </p>
          </div>
          <label className="flex items-center cursor-pointer ml-4">
            <div className="relative">
