...
- 評価落ち・違反者の判定は「基本・評価設定」の評価落ちロール・違反者ロールで行います
...
- 持っているロールを1つずつ枠で表示します。準メン・本メン＝黄色（昇格のお祝い）、評価落ち＝赤（励まし）、違反者＝赤（反省を促すメッセージ）、ショップで購入して付いたロール＝オレンジ（「ショップで○○を購入したため○○のロールを付与しました！」）、それ以外＝青（「○○のロールが付与されました！」）です。本メン・準メン・評価落ち・違反者の判定は「基本・評価設定」の「本メンバーロール」「準メンバーロール」「評価落ちロール」「違反者ロール」だけで行います（未設定なら当てはまる人はいない扱い。設定状況は「Webアクティビティ設定」の「判定に使うロール」で確認でき、未設定のものは「未設定」と表示されます）
...
- 「VCルーム設定」の「画面共有の許可」で、宿・高級宿・ゲームVC・賭博VC・カスタムVCごとに画面共有のON/OFFを設定できます（未設定はON）
- 「VCトリガー設定」でも、トリガーごとに「画面共有を許可」を設定できます（未設定はON）
- OFFの部屋・VCでは作成者も含めて画面共有できません。部屋の中のパネルに「画面共有: できます / できません」と表示されます。設定の変更は、これから作られる部屋から反映されます
- 以前のパネルにあった「画面共有を許可／禁止」ボタンは外しました。すでに送られたパネルのボタンを押すと、設定で決まるようになったことと今の状態を案内します
